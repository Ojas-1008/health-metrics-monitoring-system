/**
 * server/tests/googleFitHelper.test.js
...
describe("Google Fit Helper - Token Refresh", () => {
  ...
  describe("refreshGoogleFitToken()", () => {
    ...
    it("should validate token structure before refresh", async () => {
      // Create user with incomplete tokens
      await User.deleteMany({});
      
      const incompleteTokenUser = await User.create({
      ...
      try {
        await refreshGoogleFitToken(incompleteTokenUser._id);
        expect(true).toBe(false); // Should not reach here
      } catch (error) {
        expect(error.statusCode).toBe(401);
        expect(error.message).toContain("missing");
  ...
  describe("Error Scenarios", () => {
    ...
    it("should handle data inconsistency (connected but no tokens)", async () => {
      ...
      try {
        ...
        expect(error.statusCode).toBe(401);
        expect(error.message).toContain("missing");
        console.log(`✅ Detected inconsistency and returned 401`);
        
        // Verify user was auto-disconnected
        const updatedUser = await User.findById(testUser._id);
        expect(updatedUser.googleFitConnected).toBe(false);
        console.log(`✅ Auto-disconnected user due to inconsistency`);
      }
    });
  });
});
